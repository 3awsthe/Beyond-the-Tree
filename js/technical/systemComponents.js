var systemComponents = {
	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				...
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : '需要 ' + formatWhole(tmp[layer].requires) + ' ' + tmp[layer].baseResource + ' 以解锁 (你有 ' + formatWhole(tmp[layer].baseAmount) + ' ' + tmp[layer].baseResource + ')')
			)
			: (
    ...
    'info-tab': {
        ...
        <span v-if="modInfo.author">
            <br>
            {{modInfo.author}} 使用了 Acamaeda 的 The Modding Tree 2.6.6.2 成功制造了一坨 [数据删除]
			<br>
			<br>
			<br>
			快来踩爆他！
		...
		<br>
		<div class="link" onclick="showTab('changelog-tab')">修改日志</div><br>
		<br><br>
    ...
    'options-tab': {
        ...
        <table>
			...
			<tr>
			    <td><button class="opt">PS可爱！</button></td>
			</tr>
        </table>`
    },
